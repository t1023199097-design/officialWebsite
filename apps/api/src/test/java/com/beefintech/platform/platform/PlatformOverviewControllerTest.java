package com.beefintech.platform.platform;

import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;

import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.web.servlet.AutoConfigureMockMvc;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.ActiveProfiles;
import org.springframework.test.web.servlet.MockMvc;

@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles("test")
class PlatformOverviewControllerTest {

    @Autowired
    private MockMvc mockMvc;

    @Test
    void exposesBrandAndReservedModulesForThePublicSite() throws Exception {
        mockMvc.perform(get("/api/public/platform/overview"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.data.brandName").value("BeeFintech"))
            .andExpect(jsonPath("$.data.modules.length()").value(3))
            .andExpect(jsonPath("$.data.modules[0].enabled").value(false));
    }
}
